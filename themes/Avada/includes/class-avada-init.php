...
class Avada_Init {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_fusion_builder_load_demo_header', [ $this, 'init_fb_demos_importer' ], 20 );
	...
	public function init_fb_demos_importer() {
		...
		if ( ( ! Avada_Helper::is_post_admin_screen() 
			...
			|| ( fusion_doing_ajax() 
				&& ( ! isset( $_POST['page_name'] ) && ! isset( $_POST['header_name'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification
			return;
		}
		...
		if ( in_array( $post_type, FusionBuilder::allowed_post_types(), true ) 
			|| ( fusion_doing_ajax() && ( isset( $_POST['page_name'] ) || isset( $_POST['header_name'] ) ) ) // phpcs:ignore WordPress.Security.NonceVerification
