	...
	public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
		...
		if ( 1 === $depth && 'enabled' === $this->menu_megamenu_status ) {
			...
			if ( isset( $fusion_meta['special_link'] ) && 'fusion-woo-cart' === $fusion_meta['special_link'] && class_exists( 'WooCommerce' ) ) {
				...
				if ( ! $show_counter && 0 < WC()->cart->get_cart_contents_count() ) {
					$item->classes[] = ' fusion-active-cart-icon';
				}
			}

			if ( isset( $fusion_meta['special_link'] ) && 'fusion-woo-my-account' === $fusion_meta['special_link'] ) {
			...
			if ( isset( $fusion_meta['special_link'] ) && 'fusion-search' === $fusion_meta['special_link'] ) {
				...
				switch ( $fusion_meta['searchform_mode'] ) {
					case 'dropdown':
						...
						$item_output = '<a class="fusion-main-menu-icon" href="#" aria-label="' . $item_title_esc . '" data-title="' . $item_title_esc . '" title="' . $item_title_esc . '">';
					...
					case 'overlay':
						...
						$item_output = '<a class="fusion-main-menu-icon fusion-menu-icon-search trigger-overlay" href="#" aria-label="' . $item_title_esc . '" data-title="' . $item_title_esc . '" title="' . $item_title_esc . '"></a>';

						$searchform_markup  = get_search_form( false );
						$searchform_markup .= '<div class="fusion-search-spacer"></div>';
						$searchform_markup .= '<a href="#" class="fusion-close-search"></a>';
